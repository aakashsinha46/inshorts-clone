...
news_links = {}
prepared_links = {}
CATEGORY = {"sports":'sport', "business":'business'}
link=[]


# categorise links
def get_newsasia_news_links(soup, category):
...
def main():
   ...
   with ThreadPoolExecutor(max_workers=MAX_WORKER) as executor:
      ...
      pool = [ executor.submit(get_newsasia_news_links, get_soup(value), key) for key,value in CATEGORY.items() ]
      # on complete
   ...
   with open('{path}/malaysia/newsasia.json'.format(path=masterPath), 'w') as f:          #the output at independent_output.json
